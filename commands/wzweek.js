...
module.exports = {
    name: 'wzweek',
    description: 'Provide the user with their weekly Warzone stats',
    args: true,
    ...
    execute(message, args) {
        if (args.length !== 3) {
            ...
            API.MWweeklystats(wzUsername, API.platforms.battle).then((output) => {
                console.log(output.wz.mode.br_all);
                let KDR = output.wz.mode.br_all.properties.kdRatio;
                KDR = KDR.toFixed(2);
                exampleEmbed.title = `Warzone stats for ${wzUsername}`;
                exampleEmbed.fields[0].value = output.wz.mode.br_all.properties.wins;
                exampleEmbed.fields[1].value = output.wz.mode.br_all.properties.kills;
                exampleEmbed.fields[2].value = output.wz.mode.br_all.properties.deaths;
                exampleEmbed.fields[3].value = KDR;
                exampleEmbed.fields[4].value = output.wz.mode.br_all.properties.matchesPlayed;
            ...
            API.MWBattleData(wzUsername, API.platforms.steam).then((output) => {
                console.log(output);
                let KDR = output.wz.properties.kdRatio;
                KDR = KDR.toFixed(2);
                exampleEmbed.title = `Warzone stats for ${wzUsername}`;
                exampleEmbed.fields[0].value = output.wz.properties.wins;
                exampleEmbed.fields[1].value = output.wz.properties.kills;
                exampleEmbed.fields[2].value = output.wz.properties.deaths;
                exampleEmbed.fields[3].value = KDR;
                exampleEmbed.fields[4].value = output.wz.properties.gamesPlayed;
                return message.channel.send({ embed: exampleEmbed });
